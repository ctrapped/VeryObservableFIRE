...
def RunBinfire(snapdir,statsDir,Nsnap,output,maxima,res,tempMin=[None],tempMax=[None],densMin=[None],densMax=[None],phasetag=['AG'],rshrinksphere=5000,rminsphere=10,shrinkfactor=0.7,returnTiltedRingData=False):
    ...
    max_x = maxima[0];max_y=maxima[1];max_z=maxima[2]
...
#CALCULATE MOMENTUMS################################################################################
    ...
    Lz = np.multiply(angMom[:,0],Lhat[0]) + np.multiply(angMom[:,1],Lhat[1]) + np.multiply(angMom[:,2],Lhat[2])
    ...
    binRange=[[-max_x,max_x],[-max_y,max_y],[-max_z,max_z]]

    binned_mass_3d_cart,binedge,binnum = stats.binned_statistic_dd(sample_cart,Gmas,op,[nx,ny,nz],range=binRange)
    #binned_mom_s_cart,binedge,binnum = stats.binned_statistic_dd(sample_cart,Gmom_s,op,[nx,ny,nz],range=binRange)
    binned_mom_r_cart,binedge,binnum = stats.binned_statistic_dd(sample_cart,Gmom_r,op,[nx,ny,nz],range=binRange)
    #binned_mom_L_cart,binedge,binnum = stats.binned_statistic_dd(sample_cart,Gmom_L,op,[nx,ny,nz],range=binRange)
    binned_mom_phi_cart,binedge,binnum = stats.binned_statistic_dd(sample_cart,Gmom_phi,op,[nx,ny,nz],range=binRange)
    ...
    if writeStatsFile:
        ...
        hf_stats.create_dataset('r0',data=r_0);
        hf_stats.create_dataset('Lhat',data=Lhat);
        ...
        hf_stats.create_dataset('xbin_size',data=xbin_size);
        hf_stats.create_dataset('ybin_size',data=ybin_size);
        hf_stats.create_dataset('zbin_size',data=zbin_size);
