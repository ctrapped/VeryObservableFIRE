...
def GenerateSpectra(gMass,speciesMassFrac,dopplerVelocity,particleSize,temp,distance,impact,species,beamSize,Nspec,bandwidth,calcThermalLevels):
    ...
    mass_species,g_upper,g_lower,E_upper,E_lower,A_ul,gamma_ul,Glevels,Elevels,n_u_fraction,n_l_fraction = GetEmissionSpeciesParameters(species) #MOVE THIS INTO GENERATESPECTRA
    ...
    #mask,impact,distance = GetParticlesInSightline(losPosition, positions, particleSize, beamSize) #Do this before and only load in these particles
    #dopplerVelocity = gVel[mask]
    #del mask
    ...
    #Replace with user defined for each species. Can do each speicies separate still and allow user to peace them together if desired in post
    nu_0 = nu_ul
...
def GetColumnDensityAlongLOS(r,impact,N_mol_in_particle,beamRadiusPhysical):
    ...
    #colDensParticle = np.multiply(np.divide(N_mol_in_particle , vol) , r)
...
def MakeSpectrum(upperToLowerRate,attenuationCrossSection,distance,dopplerVelocity,temp,colDens,pathLength,nu_ul,f_lu,gamma_ul,spectralRange,mass_species,beamRadiusPhysical,Nspec,species):
    ...
    #print("Starting Particle Loop for ",Nparticles," particles:")
    t0 = time.time()
    for p in range(0,Nparticles): #This form must be executed sequentially, as absorption is built up
        sigma_nu = GenLineProfile(nu_ul, f_lu, gamma_ul, dopplerVelocity[p], spectralRange, temp[p], mass_species, Nspec)
        #phi_nu = sigma_nu * pi * e**2 * f_lu / m_e / c #initial typo
        phi_nu = sigma_nu * m_e * c / pi / (e**2) / f_lu
        #power_emission = (upperToLowerRate[p] * (beamAreaPhysical[p] * pathLength[p]) * h) * np.multiply(nu,phi_nu)
        if distance[p]==0:
            distance[p]=eps;
            
       # lineTerm = np.multiply(nu,phi_nu)
        
        power_emission = (upperToLowerRate[p] * h) * np.multiply(nu,phi_nu)
        #powerNormalizationFactor=ColumnToEmissionPower(colDens , species) / np.sum(power_emission)

        emission += power_emission/ distance[p]**2
        #spectra += power_emission/ distance[p]**2 * np.exp(-optical_depth)
        spectra = np.add(spectra , np.multiply(power_emission , np.exp(-optical_depth)) / distance[p]**2)
        #optical_depth += phi_nu*attenuationCrossSection[p]*pathLength[p]
        ...
        optical_depth += optical_depth_particle

    return spectra,emission,optical_depth,nu

#def ColumnToEmissionPower(colDens,species):
 #   if species=='HI_21cm':
...
def GenLineProfile(nu_ul,f_lu,gamma_ul,dopplerVelocity,spectralRange,temp,mass_species,Nspec):
    #define absorption line profile here
    ...
    #doppler_freq_shift = -nu_ul  * np.divide(dopplerBeta , (dopplerBeta + 1)) 
    #doppler_index_shift = int(np.round(  -nu_ul/spectral_resolution  * np.divide(dopplerBeta , (dopplerBeta + 1))*Nspec ))
    ...
    #print("nu_ul=",nu_ul," spec res=",spectral_resolution," beta=",dopplerBeta)
    #print("Doppler Velocity=",dopplerVelocity," Dopper index shift=",doppler_index_shift)

    #v = (np.divide(nu_ul+doppler_freq_shift , nu) -1 ) * c #in cm/s
    v = np.divide( 1 - np.power(nu / (nu_ul+doppler_freq_shift) , 2) , 1 + np.power(nu / (nu_ul+doppler_freq_shift) , 2)) * c #in cm/s
    #doppler_index_shift = int(np.round( ((dopplerVelocity * nu_ul / c)) / spectral_resolution ))
    #peak_index = int(np.round( (nu_ul-spectralRange[0]) / spectral_resoultion))

    #u_v = spectra[peak_index+doppler_index_shift]
    #nbar_gamma = c^3 / (8*pi*h*v^3) * u_v #cm^3  eV-1 * units of u_v ---> unitless

    lamda_ul = c / nu_ul 

    #sigma_f = np.sqrt(kb*temp/mass_species) 
    #b = np.sqrt(2.)*sigma_f
    b=12.90*np.sqrt(temp*np.power(10.,-4) / (mass_species/amu))*1000.*100. #cm/s
    b6 = b / (10.*1000.*100.) #b/10 km/s unitless
    #core = 1 / np.sqrt(pi) * 1 / f_ul * c/b * np.exp(-v*v/b/b)
    prefactor = np.sqrt(pi)  * (e**2)/(m_e*c) * f_lu*lamda_ul / b 

    z = np.sqrt(10.31+np.log(7616 / (gamma_ul*lamda_ul)*b6)) 
    transition_v = np.abs(b*z)

   # print("Transition v is:",transition_v)
   # print("min/max v is:",np.min(v),np.max(v))


    #coreFreqs = np.add( np.where(np.abs(v)<=transition_v) , doppler_index_shift)[0] 
    #wingFreqs = np.add( np.where(np.abs(v)> transition_v) , doppler_index_shift)[0]
    ...
    #if (np.size(coreFreqs)>0):
    #    print "Core Frequencies Resolved!",np.size(coreFreqs)
    wingFreqs = wingFreqs[((wingFreqs<np.size(nu)) & (wingFreqs>=0))]


    #print "There are: ",np.size(coreFreqs),"Core Freqs.","z=",z,"b=",b,"transition=",transition_v


    #transitionFreq = f_ul - (f_ul * v / c)

    sigma = np.zeros(np.shape(nu))
    sigma[coreFreqs] = prefactor * np.exp(-np.power(v[coreFreqs],2) / b**2)
    #if np.size(sigma[coreFreqs])>0:
      #print("Max of Gaussian = ",prefactor,". Max Resolved = ",np.max(sigma[coreFreqs]))
     # if (prefactor/np.max(sigma[coreFreqs]) > 10):
       # print("Warning peak is under resolved by a factor of ",prefactor/np.max(sigma[coreFreqs]))
        
      #sigma[np.argmax(sigma)]=prefactor; #force peak to be resolved?
...
def GenEmissionAndAbsorptionRates(colDens,T,species,beamRadiusPhysical,calcThermalLevels):
    #Need to convert beamSize to a physical scale
    ...
    if n_u_fraction is None or calcThermalLevels:
        ...
        #print("CORRECTLY ENTERED THE ELSE STATEMENT FOR N_U_FRACTION")
    ...
    #n_l = np.multiply(n_total  , np.exp( np.divide(-E_l , kb*T))) * g_l / partitionFunction


    #u_v = #Need radiation transfer simulations for this.
    #nbar_gamma = c^3 / (8*pi*h*v^3) * u_v #cm^3  eV-1 * units of u_v ---> unitless

    upperToLowerRate = n_u*A_ul# * (1+nbar_gamma) #Hz
    #lowerToUpperRate = n_l * g_u/g_l * A_ul * nbar_gamma #Hz
